...
#
# test runner based on docker/podman.
#
if [ "${CI}" != "true" ]; then
...
COMPOSER_ROOT_VERSION="1.0.8-dev"
...
case ${TEST_SUITE} in
    ...
    checkRst)
        if [[ -d "Documentation" ]]; then
            ...
            echo ">> No documentation found, skip validating ReST files."
            SUITE_EXIT_CODE=0
        fi
        ;;
    checkExceptionCodes)
    ...
    lintTypoScript)
        ...
        ;;
    renderDocumentation)
        ${CONTAINER_BIN} run ${DOCUMENTATION_COMMON_PARAMS} --pull always ghcr.io/typo3-documentation/render-guides:latest --config=Documentation
        SUITE_EXIT_CODE=$?
        ;;
    phpstan)
